<?php
	...
	for ($i = 1; $i < 51; $i++) {
		...
		if (file_exists("../../cache/" . $row[$i] . "/1.html") && time() - $cachetime < filemtime("../../cache/" . $row[$i] . "/1.html")) {
			...
			$html = file_get_html("http://shmotki.net/catalog/zhenskaya-odezhda/platya"); // Берем ссылку на список товаров
...
$html = file_get_contents('https://bfide.ru/catalog/ekipirovka/');
$saw = new nokogiri($html);

foreach ($saw->get('.product_name a') as $link){
    echo $link['href'];
    echo '<br/>';
}

//var_dump($saw->get('.product_name a')->toArray());
